...
/**
 * A SwingWorker that uses a path finding algorithm to find a shortest path in a worker thread.
 * 
    ...
    implements StreetSegmentObserver
{
   ...
   *          The StreetNetwork
   * @param document
   *          The CartographyDocument
   * @param panel
   *          The CartographyPanel
   */
  public PathFindingWorker(final ShortestPathAlgorithm alg, final int origin, final int destination,
      final StreetNetwork net, final CartographyDocument<StreetSegment> document,
  ...
  {
    Map<String, StreetSegment> output = alg.findPath(origin, destination, net);
  ...
  /**
   * Determines whether intermediate results should be displayed on the map.
   * 
   * @param show
   *          true to show intermediate results; false otherwise
   */
  public void shouldShowIntermediateResults(final boolean show)
  {
    if (show)
      alg.addStreetSegmentObserver(this);
    else
      alg.removeStreetSegmentObserver(this);
